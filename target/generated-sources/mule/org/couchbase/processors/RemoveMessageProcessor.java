...
/**
 * RemoveMessageProcessor invokes the {@link org.couchbase.CouchbaseConnector#Remove(java.lang.String)} method in {@link CouchbaseConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
        ...
        throws MuleException
    {
        ...
        try {
            if (bucketName!= null) {
                _transformedBucketName = ((String) evaluateAndTransform(_muleMessage, RemoveMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            } else {
                if (_castedModuleObject.getBucketName() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("Remove"), event, new RuntimeException("You must provide a bucketName at the config or the message processor level."));
                }
                _transformedBucketName = ((String) evaluateAndTransform(_muleMessage, RemoveMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, _castedModuleObject.getBucketName()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, RemoveMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("Remove"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, RemoveMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedKey = ((String) evaluateAndTransform(_muleMessage, RemoveMessageProcessor.class.getDeclaredField("_keyType").getGenericType(), null, key));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("Remove"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            connection.Remove(_transformedKey);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
