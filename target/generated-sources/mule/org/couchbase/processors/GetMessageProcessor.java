...
/**
 * GetMessageProcessor invokes the {@link org.couchbase.CouchbaseConnector#Get(java.lang.String)} method in {@link CouchbaseConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("Get"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (bucketName!= null) {
                _transformedBucketName = ((String) evaluateAndTransform(_muleMessage, GetMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, bucketName));
            } else {
                if (_castedModuleObject.getBucketName() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("Get"), event, new RuntimeException("You must provide a bucketName at the config or the message processor level."));
                }
                _transformedBucketName = ((String) evaluateAndTransform(_muleMessage, GetMessageProcessor.class.getDeclaredField("_bucketNameType").getGenericType(), null, _castedModuleObject.getBucketName()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, GetMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("Get"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, GetMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedKey = ((String) evaluateAndTransform(_muleMessage, GetMessageProcessor.class.getDeclaredField("_keyType").getGenericType(), null, key));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("Get"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.Get(_transformedKey);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("Get"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
